 ...
 */
function magxpress_customize_register( $wp_customize ) {
	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial(
			...
			array(
				...
				'render_callback' => 'magxpress_customize_partial_blogname',
			)
		);
		$wp_customize->selective_refresh->add_partial(
			'blogdescription',
			array(
				...
				'render_callback' => 'magxpress_customize_partial_blogdescription',
			)
		);
	}

	/*Get default values to set while building customizer elements*/
	$default_options = magxpress_get_default_customizer_values();
	/* Header Background Color*/
	$wp_customize->add_setting(
	    'magxpress_options[header_bg_color]',
	    array(
	...
	$wp_customize->add_control(
	    new WP_Customize_Color_Control(
	        ...
	        'magxpress_options[header_bg_color]',
	        array(
	            'label' => __('Header Background Color', 'magxpress'),
	            'section' => 'colors',
	            'type' => 'color',
        		'priority' => 1,
    ...
    require_once get_template_directory() . '/inc/customizer/theme-options/header-featured-post.php';
    ...
    require_once get_template_directory() . '/inc/customizer/theme-options/top-widget.php';
    require_once get_template_directory() . '/inc/customizer/theme-options/category.php';
	require_once get_template_directory() . '/inc/customizer/theme-options/3-column-widget.php';
	require_once get_template_directory() . '/inc/customizer/theme-options/article-with-separator.php';
	require_once get_template_directory() . '/inc/customizer/theme-options/2-column-widget.php';
	require_once get_template_directory() . '/inc/customizer/theme-options/separator_between_homepage_widget.php';
	require_once get_template_directory() . '/inc/customizer/theme-options/fullwidth-widget.php';
	...
	$wp_customize->add_section( 'pro__section', array(
		'title'       => '' . esc_html__( 'View PRO Version', 'magxpress' ),
		'priority'    => 2,
		'description' => sprintf(
			...
			sprintf( '<a href="%1$s" target="_blank" class="button button-primary">%2$s</a>', esc_url( magxpress_get_pro_link() ), esc_html__( 'View Magxpress PRO', 'magxpress' ) )
		),
	) );
	$wp_customize->add_setting( 'magxpress_style_view_pro_desc', array(
		'default'           => '',
		'sanitize_callback' => '__return_true',
	) );
	$wp_customize->add_control( 'magxpress_style_view_pro_desc', array(
		'section' => 'pro__section',
		'type'    => 'hidden',
	) );
}
add_action( 'customize_register', 'magxpress_customize_register' );
 ...
 * @return void
 */
function magxpress_customize_partial_blogname() {
	bloginfo( 'name' );
}
 ...
 * @return void
 */
function magxpress_customize_partial_blogdescription() {
 ...
 */
function magxpress_customize_preview_js() {
    wp_enqueue_script( 'magxpress-customizer', get_template_directory_uri() . '/assets/js/customizer.js', array( 'customize-preview' ), MAGXPRESS_S_VERSION, true );

}
add_action( 'customize_preview_init', 'magxpress_customize_preview_js' );
 ...
 * @since 1.0.0
 */
function magxpress_customizer_control_scripts(){
    wp_enqueue_style('magxpress-customizer-css', get_template_directory_uri() . '/assets/css/customizer.css');
    wp_enqueue_script('magxpress-customizer-controls', get_template_directory_uri() . '/assets/js/customizer-admin.js', array('jquery', 'jquery-ui-sortable', 'customize-controls') );
}
add_action('customize_controls_enqueue_scripts', 'magxpress_customizer_control_scripts', 0);

/**
 * Generate a link to the premium theme info page.
 */
function magxpress_get_pro_link() {
	return 'https://www.themeinwp.com/theme/magxpress-pro/';
}